create procedure syn.usp_ImportFileCustomerSeasonal
	@ID_Record int -- 1) Для переменных используется PascaleCase -> @IDRecord
AS
set nocount on
begin
...
/*  2) Алиас должен составляться из первых букв составного ключа.
	В данном случае получается if (системное слово), что приводит к исключению -
	в алиас добавляется первая согласная первого слова. -> imf
*/
	...
	select
		...
		,cst.ID as ID_CustomerSystemType -- 3) Нет префикса источника
		,s.ID as ID_Season -- 3
		,cast(cs.DateBegin as date) as DateBegin -- 3
		,cast(cs.DateEnd as date) as DateEnd -- 3
		,c_dist.ID as ID_dbo_CustomerDistributor
		,cast(isnull(cs.FlagActive, 0) as bit) as FlagActive
	into #CustomerSeasonal -- 4) Название временной таблицы должно быть хотя-бы {Название}[_Постфикс]
	from syn.SA_CustomerSeasonal cs
		...
		/* Нарушена логика запроса. dbo.Customer.UID_DS это либо id покупателя либо id дистрибьютора.
		   Здесь же джойн сначала по id Customer'а а потом по id Distributor'а */
	...
	begin
		...
		from #BadInsertedRows as bir -- Зачем алиас, если джоинов нет?
